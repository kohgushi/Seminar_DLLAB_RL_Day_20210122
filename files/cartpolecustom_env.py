...
# オリジナルの CartPole を継承してカスタムクラスを作成します
class CartPoleCustom(CartPoleEnv):
    def __init__(self, friction_cart, friction_pole, pole_length):
        super().__init__()
        self.Nc_pre = 1
        self.force_mag = 10.0
        self.step_count = 0
...
def create_env(config):
    friction_cart = config["friction_cart"]
    friction_pole = config["friction_pole"]
    pole_length = config["pole_length"]

    print(f"friction_cart: {friction_cart}")
    print(f"friction_pole: {friction_pole}")
    print(f"pole_length: {pole_length}")
    
    env = CartPoleCustom(friction_cart, friction_pole, pole_length)
